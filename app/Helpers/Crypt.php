...
class Crypt
{
    ...
    {
        $number = 256; //filter_var($this->encryptMethod, FILTER_SANITIZE_NUMBER_INT)
    ...
    {
        $crypt = new Crypt;
        $crypt->encryptMethod = $cipherMethod;
    }// setCipherMethod
    ...
    public static function decrypt($encryptedString)
    {
        ...
        // print_r($crypt->encryptMethodLength());exit;
        ...
        $hashKey = hash_pbkdf2('sha512', $key, $salt, $iterations, ($crypt->encryptMethodLength() / 4));
        unset($iterations, $json, $salt);

        $decrypted= openssl_decrypt($cipherText , $crypt->encryptMethod, hex2bin($hashKey), OPENSSL_RAW_DATA, $iv);
        unset($cipherText, $hashKey, $iv);
        $decrypted = preg_replace('/[\x00-\x1F\x7F]/u', '' ,$decrypted);
        return $decrypted;
    }// decrypt
    ...
    public static function encrypt($string)
    {
        ...
        $ivLength = openssl_cipher_iv_length($crypt->encryptMethod);
        ...
        $encryptedString = openssl_encrypt($string, $crypt->encryptMethod, hex2bin($hashKey), OPENSSL_RAW_DATA, $iv);
