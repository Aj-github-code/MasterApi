...
use App\Models\QNAMgmtUserExamResult;
use App\Models\QNAMgmtUserExams;
...
class CampaignController extends Controller
{
    ...
    public function getCampaignByReferenceID(Request $request){
        ...
        if($validator->fails()){
            ...
            $campaign = QNAMgmtCampaign::where('other_parameter' , 'LIKE', '%"reference_no":"'.$postData['reference_no'].'"%')->orderBy('id', 'desc')->first();
    ...
    public function getCampaignByCampaignCode(Request $request){
        ...
        if($validator->fails()){
            ...
            $campaign = QNAMgmtCampaign::where('campaign_code' , 'LIKE', $postData['campaign_code'])->orderBy('id', 'desc')->first();
    ...
    public function getCampaign(Request $request){
        $companyTablePrefix =  Helper::getCompany();
        
       // echo $companyTablePrefix;exit;
        $searchQuery = "";
        $response = array();
        $columnName = 'id';
        ...
        $query = QNAMgmtCampaign::where('campaigns.is_active', '1');
            // ->where('campaigns.other_parameter', 'LIKE', '%roles":{%'.$user_role[0]->slug.'%');
        ...
        if($_SERVER['REQUEST_METHOD']=='POST'){
            ...
            if($searchValue != ''){
               $searchQuery = '(campaign_code LIKE "%'.$searchValue.'%" or other_parameter LIKE `%"reference_no":"'.$searchValue.'%"` or other_parameter  like `%"title":"'.$searchValue.'%"`)';
            }
            //print_r($request->post('campign_type'));exit;
            if(NULL!==$request->post('campaign_type')){
                $query->where('campaign_type', 'LIKE', $request->post('campaign_type'));
            }
            
            $query->join($companyTablePrefix.'assign_campaign as ac', 'ac.campaign_code', 'LIKE', 'campaigns.campaign_code', 'left')->
                ...
                select(['campaigns.*', 'ac.user_id', 'ac.created_by as assigned_by', 'ac.status', 'ar.role_name', 'au.name as assigned_name']);
            ...
            if(isset($postData['search']) && (NULL !== $postData['search'])){
               $query->where('campaigns.other_parameter', "LIKE", '%"title":"'.$postData['search'].'%');
            }
        }
        
        $query->GroupBy('campaigns.campaign_code');
        ...
        foreach($records as $keys => $row){
            ...
            $user_prev_exam = QNAMgmtUserExamResult::where('user_id' , 'LIKE', auth()->id())->orderBy('created', 'desc')->first();
    ...
    public function createUserExamResult(Request $request){
        ...
        $check = QNAMgmtUserExamResult::
            ...
            $insert = QNAMgmtUserExamResult::create($data);
    ...
    public function submitAnswer(Request $request) {
        ...
        $insert = QNAMgmtUserExams::create($data);
    ...
    protected function examresultstatus($exam_code,$campaign_code,$marks){
        $examlastinserted = QNAMgmtUserExamResult::where('exam_code' , 'LIKE', $exam_code)->orderBy('id', 'desc')->first();
        
        $marks = $marks +$examlastinserted->marks;
        //print_r($marks);exit;
        $getExamCampaignrecord = QNAMgmtCampaign::select('other_parameter')->where('campaign_code' , 'LIKE', $campaign_code)->orderBy('id', 'desc')->first();
        ...
        $updateExamResult = QNAMgmtUserExamResult::where('exam_code',$exam_code)->update(['marks'=>$marks,'status'=>$status]);
        return true;
            ...
            foreach($mainData as $key => $table){
                ...
                foreach($table as $tkey => $columns){
                        ...
                        if($tkey !== 'campaign_answers'){
                            ...
                            if($ckey === 'answers'){
                                foreach($rows as $rkeys => $answers){
                                    ...
                                    $insertAnswer = DB::table($companyTablePrefix.$tkey)->insert($inputAnswer);
                                }
                            }
                        }
                    } 
                    if($tkey !== 'campaign_answers'){
                        ...
                        $insert = DB::table($companyTablePrefix.$tkey)->insertGetId($input);
    ...
    public function UserExamResultList(Request $request){
        ...
        $result = DB::table($companyTablePrefix.'user_exam_results  user_exam_results')->join($companyTablePrefix.'user_exams  user_exams', 'user_exam_results.exam_code', '=', 'user_exams.exam_code')->where('user_exam_results.exam_code' , 'LIKE', $exam_code)->orderBy('user_exam_results.id', 'desc')->get();//echo "<pre>";print_r($result);exit;
        return $result;
    }
    ...
    public function getCampaign2(Request $request){
        ...
        $user_role = $request->user()->roles()->get();
        DB::enableQueryLog();
        $campaign = QNAMgmtCampaign::where('is_active', true);
        ...
        $query = QNAMgmtCampaign::where('c.is_active', '1')
        ...
        if($_SERVER['REQUEST_METHOD']=='POST'){
            ...
            if(isset($postData['assigned']) && (NULL !== $postData['assigned'])){
                $query->join($companyTablePrefix.'assign_campaign as ac', 'ac.campaign_code', 'LIKE', 'c.campaign_code', 'left')->
                    join('users as au', 'au.id', '=', 'ac.user_id', 'left')->
                    join('user_roles as aur', 'aur.user_id', '=', 'au.id', 'left')->
    ...
    public function ExamResultList(Request $request) {
        ...
        $query = QNAMgmtUserExamResult::select(['users.*', 'user_exam_results.id as exam_result_id', 'exam_code', 'campaign_code', 'marks', 'user_exam_results.status'])
    ...
    function generateCampaignCode($type = 'faq') {
        ...
        } while (QNAMgmtCampaign::where("campaign_code", "LIKE", $type.'-'.$code)->first());
    ...
    public function uploadDocsCampaign(Request $request){
        ...
        try{
            ...
            $msg = '';
            $up_record = [];
            $getExamCampaignrecord = QNAMgmtCampaign::select('other_parameter')->where('campaign_code' , 'LIKE', $input['campaign_code'])->orderBy('id', 'desc')->first();
            $campaign_other_param = json_decode($getExamCampaignrecord->other_parameter);
            ...
            //echo "<pre>";print_r($up_record);exit;
            $updateExamResult = QNAMgmtCampaign::where('campaign_code',$input['campaign_code'])->update(['other_parameter'=>json_encode($campaign_other_param)]);
    ...
    public function UserExamList(Request $request){
        ...
        try{
            ...
            $result = DB::table($companyTablePrefix.'user_exams')->where('exam_code' , 'LIKE', $exam_code)->orderBy('id', 'desc')->get();//echo "<pre>";print_r($result);exit;
    ...
    public function UserExamCampaignList(Request $request){
        try{
            ...
            $query = QNAMgmtUserExamResult::select('*');
            if(isset($postData['exam_code']) && (NULL !== $postData['exam_code'])){
            ...
            if($result){
                ...
                foreach($result as $keys => $row){
                    $campaign_detail = QNAMgmtCampaign::where('campaign_code',$row->campaign_code)->first();
